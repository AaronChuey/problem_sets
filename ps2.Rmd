...
rating_hyp_long_d = rating_hyp_d %>%
  pivot_longer(cols = "Game1Angry1":"Game4Neutral3", names_to = "Measurement", values_to = "Rating")
```
...
```{r summary with standard errors}
library(plotrix)
performance_diff_summary_d = performance_diff_d %>%
  group_by(ConfrontationalGame, MusicCondition) %>%
  summarize(MeanImprovementScore = mean(ImprovementScore, na.rm=T),
            sem_ImprovementScore = std.error(ImprovementScore, na.rm=T))
```
...
```{r TamirFig2 with error bars}
#with error bars
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) +
  geom_bar(position="dodge", stat="identity") + 
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin = (MeanImprovementScore - sem_ImprovementScore), ymax = (MeanImprovementScore + sem_ImprovementScore)), position = "dodge")
```
